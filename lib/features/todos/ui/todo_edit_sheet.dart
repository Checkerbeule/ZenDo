...
class TodoEditSheet extends StatefulWidget {
  const TodoEditSheet.editTodo({super.key, required this.todo})
    : listScope = null;

  const TodoEditSheet.newTodo({super.key, required this.listScope})
    : todo = null;
...
class _TodoEditSheetState extends State<TodoEditSheet> {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      setState(() {
        ...
        expirationDateController.text =
            ...
            todoService.calcExpiry(selectedScope)?.formatYmD(locale) ??
            ' - ';
      });
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final AppSettingsService settingsService = context
        .read<AppSettingsService>();
    final List<DropdownMenuItem<ListScope>> listScopeDropDownItems =
        settingsService
            .getActiveListScopes()
            .map(
                  ...
                  children: [
                      ...
                      IconButton(
                        ...
                        onPressed: () async {
                          if (todo != null) {
                            ...
                            final successful = await todoService
                                .markAsCompleted(todo!.uuid);
                            if (successful) {
                              nav.pop();
                            } else {
                              logger.e(
                    ...
                    IconButton(
                      ...
                      onPressed: () async {
                        ...
                        if (delete != null && delete && todo != null) {
                          final deleted = await todoService.delete(todo!);
                          if (deleted == 1) {
                            nav.pop();
                          } else {
                    ...
                    Expanded(
                      child: TextFormField(
                        ...
                        validator: (value) {
                          ...
                          final fittingScope = todoService.calcFittingScope(
                            selectedDate,
                          );
